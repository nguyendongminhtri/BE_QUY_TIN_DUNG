...
public class StorySuccessController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> updateStorySuccess(@PathVariable Long id, @RequestBody StorySuccessEntity updatedData) {
        Optional<StorySuccessEntity> optionalNews = storySuccessService.findById(id);
        if (!optionalNews.isPresent()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new ResponMessage("story_success_not_found"));
        }
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<?> deleteStorySuccess(@PathVariable Long id) {
    ...
    public ResponseEntity<?> updateStorySuccessStatus(
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Không tìm thấy story với ID: " + id);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Lỗi khi cập nhật trạng thái");
        }
    }
    @GetMapping("/by-category/{categoryId}")
    public Page<StorySuccessEntity> getStorySuccessByCategory(@PathVariable Long categoryId, Pageable pageable) {
